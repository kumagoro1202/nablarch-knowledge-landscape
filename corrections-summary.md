...
cmd_471 で FW 責任範囲を是正したが、「実装者責任範囲」の中に依然として
...
## cmd_464 目標(15以上)との比較

cmd_464 の目標値「処理方式ごとに 15 観点以上」と最終観点数を比較:

| 処理方式 | cmd_472後 | cmd_464目標 | 達成状況 |
|---------|-----------|-------------|----------|
| バッチ | 7 | 15以上 | **未達**（-8） |
| Web | 13 | 15以上 | **未達**（-2） |
| REST | 11 | 15以上 | **未達**（-4） |
...
cmd_464 の目標値「15以上」は普遍性フィルタ適用前の数値である。
cmd_471（FW責任範囲除外）と cmd_472（プロジェクト固有事情除外）の
